...
function showSlot(slot) {
	$jq('#countAdNumberAndTime').attr('data-countadnumber' , slot.name);
	$jq('#countAdNumberAndTime').attr('data-countadtime' , slot.time);
	
	/*
	var videoSkipTime = $jq('#videoSkipTime').val();
	setTimeout(function() {
		$jq(".skipBtn").show().addClass("disabled");
	  }, videoSkipTime);	
	*/
    ...
    if($jq('#example_video_1').get(0).currentTime > 0.25){
      	//$jq(".skipBtn").removeClass("disabled");
...
$jq(document).on('click','.skipBtn', function(e){	
    jQuery('.skipBtn').hide();
